...
static int (*p_cuMemExportToShareableHandle)(void*,
                                             CUmemGenericAllocationHandle, int,
                                             unsigned long long);
static int (*p_cuMemImportFromShareableHandle)(CUmemGenericAllocationHandle*,
                                               void*, int);
...
int main(int argc, char** argv) {
  if (argc != 2) die("usage: mcshim-helper <sockfd>");
  int s = atoi(argv[1]);

  void* lib = dlopen("libcuda.so.1", RTLD_NOW);
  if (!lib) die("dlopen libcuda.so.1 failed");
  ...
  p_cuMemExportToShareableHandle = dlsym(lib, "cuMemExportToShareableHandle");
  ...
  for (;;) {
    ...
    if (r <= 0) _exit(0); /* EOF: parent went away */
    if (!strncmp(buf, "CREATE ", 7)) {
      CUmulticastObjectProp prop;
      memset(&prop, 0, sizeof(prop));
      unsigned long long ht = 0, fl = 0;
      if (sscanf(buf + 7, "%u %zu %llu %llu", &prop.numDevices, &prop.size, &ht,
                 &fl) != 4) {
        reply_err(s, -1);
        continue;
      }
      ...
      int fd = -1;
      rc = p_cuMemExportToShareableHandle(&fd, group,
                                          CU_MEM_HANDLE_TYPE_POSIX_FD, 0);
      if (rc != 0) {
        reply_err(s, rc);
        continue;
      }
      ...
      rc = p_cuMemImportFromShareableHandle(&group, (void*)(long)rfd,
