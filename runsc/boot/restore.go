...
func (r *restorer) restore(l *Loader) error {
	...
	// Load the state. Use the same monotonic clock source as at boot (see
	// hostAlignedMonotonicForProfiling); the Timekeeper's host-alignment flag
	// is restored from the state file.
	clocks := time.NewCalibratedClocks()
	if hostAlignedMonotonicForProfiling(l.root.spec, l.root.conf) {
