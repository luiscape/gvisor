...
func RegisterFlags(flagSet *flag.FlagSet) {
	...
	flagSet.Var(CUDAMulticastShimSourceImage.Ptr(), "cuda-multicast-shim-source", "where the multicast suspend/resume interposer comes from: IMAGE (the container image carries it at --cuda-multicast-shim-path, which enables it) or EMBEDDED (write the interposer bundled inside the runsc binary -- mcshim.so and its companion mcshim-helper -- into each GPU container's filesystem at container creation, at --cuda-multicast-shim-path, default "+DefaultCUDAMulticastShimPath+", so the image need not carry it; the write lands in the rootfs overlay under the default --overlay2 configuration, never in the user's rootfs on the host). Applied per container.")
