...
func RegisterFlags(flagSet *flag.FlagSet) {
	...
	flagSet.String("cuda-checkpoint-path", "", "path to the cuda-checkpoint binary inside the container. When set for a GPU container on driver R610+, the container's command is wrapped in `cuda-checkpoint --launch-job` so its CUDA processes share a job and CUDA IPC state can be checkpointed/restored coherently. Applied per container. Pair with `runsc checkpoint --cuda-checkpoint-sequential`, as job members must be toggled sequentially.")
