...
func RegisterFlags(flagSet *flag.FlagSet) {
	...
	flagSet.Var(SidecarReleaseNever.Ptr(), "sidecar-release-enforcement-policy", "when spawned sidecar binaries must match runsc's release: NEVER, ALWAYS, or IF_RELEASE_BUILD. May be overridden by setting GVISOR_ENFORCE_RELEASE=SKIP as env var.")
	...
	flagSet.String("cuda-multicast-shim-path", "", "path to the multicast suspend/resume interposer (mcshim.so) inside the container. When set for a GPU container on driver R550+, the container's command is run with the interposer LD_PRELOADed (via environment and /etc/ld.so.preload). It makes a process holding multicast (NVLS / symmetric-memory) objects checkpointable: gVisor has it release them before the checkpoint and rebuild them at identical virtual addresses after restore, so captured CUDA graphs stay valid. Applied per container.")
	flagSet.Bool("cuda-multicast-shim-embedded", false, "write the multicast suspend/resume interposer bundled inside the runsc binary (mcshim.so and its companion mcshim-helper) into each GPU container's filesystem at container creation, at --cuda-multicast-shim-path (default "+DefaultCUDAMulticastShimPath+"), so the container image does not need to carry the interposer. The write lands in the rootfs overlay under the default --overlay2 configuration, never in the user's rootfs on the host. Applied per container.")
