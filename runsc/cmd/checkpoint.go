...
func (c *Checkpoint) SetFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&c.cudaMulticastSuspend, "cuda-multicast-suspend", false, "EXPERIMENTAL: suspend NVLS/symmetric-memory multicast objects in nvproxy across checkpoint/restore instead of treating them as a hard blocker. Requires cuda-checkpoint job semantics (driver R610+); on older drivers the restore toggle fails to restore a process that held a multicast object.")
