...
func (c *Container) Restore(conf *config.Config, imagePath string, direct, background, adoptPagesFile bool, networkArgs *boot.CreateLinksAndRoutesArgs) error {
	log.Debugf("Restore container, cid: %s", c.ID)

	restore := func(conf *config.Config, spec *specs.Spec) error {
		return c.Sandbox.Restore(conf, spec, c.ID, imagePath, direct, background, adoptPagesFile, networkArgs)
	}
