...
unload_nvidia_modules() {
  ...
  for _ in 1 2 3; do
    ...
    # nvidia_fs (GPUDirect Storage) is a user of the base nvidia module; it
    # must be removed or `rmmod nvidia` fails with "Module nvidia is in use".
    sudo modprobe --remove nvidia_fs 2>/dev/null || true
...
# Distro family detection. RHEL-family (Oracle Linux, Rocky, Amazon Linux,
# RHEL, CentOS, Fedora) uses dnf/yum + dracut; Debian-family uses apt +
# update-initramfs.
OS_ID="$(. /etc/os-release; echo "$ID")"
case "$OS_ID" in
  ol|rocky|amzn|rhel|centos|fedora) IS_RHEL_FAMILY=1 ;;
  *) IS_RHEL_FAMILY=0 ;;
esac
